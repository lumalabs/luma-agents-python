...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `luma_agents.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `luma_agents.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `luma_agents.APIError`.
...
except luma_agents.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except luma_agents.APIStatusError as e:
